...
export function useAsyncValue<T>(
    ...
    const [callState, setCallState] = useState<AsyncValueState<T>>({
        loaded: false,
    })
...
export const useAsyncStore = <T extends AsyncStore>(
    store: T
): AsyncValueState<Values<T>> => {
    if (!allLoaded(store)) {
        return { loaded: false }
    }
    ...
    return {
        ...
        value: Object.fromEntries(
            Object.entries(store).map(([property, asyncValueState]) => [
                property,
                asyncValueState.value,
            ])
        ) as Values<T>,
    }
}
